 ...
 */
export function transformLinks(content, baseUrl, currentUrlPath) {
 ...
 */
export function transformSingleLink(href, baseUrl, currentUrlPath) {
    // Skip external links
 ...
 */
function resolveRelativeUrl(href, currentUrlPath) {
    // Already absolute
    if (href.startsWith('/')) {
        return href;
    }
 ...
 */
export function validateLinks(content, availablePaths, currentPath) {
    ...
    for (const href of links) {
        ...
        const exists = variants.some((v) => availablePaths.has(v) || availablePaths.has(v.replace(/^\//, '')));
        if (!exists) {
...
//# sourceMappingURL=link-transformer.js.map