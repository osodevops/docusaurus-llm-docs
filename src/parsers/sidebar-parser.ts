...
import type { Sidebar, SidebarItem, DocSection, DocPage } from '../types/index.js';
 ...
 */
export async function parseSidebar(sidebarPath: string): Promise<DocSection[]> {
  const absolutePath = path.resolve(sidebarPath);
  const fileUrl = pathToFileURL(absolutePath).href;

  let sidebarModule: Sidebar | { default: Sidebar };
  ...
  const sidebar: Sidebar =
    'default' in sidebarModule ? (sidebarModule.default as Sidebar) : sidebarModule;
...
): { type: 'page'; page: DocPage } | { type: 'section'; section: DocSection } | { type: 'skip' } {
 ...
 */
export function flattenSections(sections: DocSection[]): DocPage[] {
  const pages: DocPage[] = [];

  function traverse(section: DocSection): void {
 ...
 */
export function countPages(sections: DocSection[]): number {
